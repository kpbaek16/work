...
function AddData() {
    ...
    var bum_table = document.getElementById('KS05table'); // 테이블지정
    ...
    bum_td2.innerHTML = "<input name='JJDKKS_WAY' type='text' class='textfield' value=''><label for='JJDKKS_WAY'></label>";
    bum_td3 = document.createElement('TD');    // Td를 생성한다.
    bum_td3.innerHTML = "<input name='JJDKKS_PART' type='text' class='textfield' value=''><label for='JJDKKS_PART'></label>";
    bum_td4 = document.createElement('TD');    // Td를 생성한다.
    bum_td4.innerHTML = "<input name='JJDKKS_RSLT' type='text' class='textfield' value=''><label for='JJDKKS_RSLT'></label>";
    var a;
    ...
    a += "<input type='hidden' name='JJDKKS_RSLTDC' value=''/>";
...
function prc_Save() {
    ...
    if (confirm("저장 하시겠습니까 ?")) {
        form.action = "/treat/0406/InspctMgr_K05_Save.do";
        form.submit();
    }
}
function prc_delete() {
    ...
    if (confirm("삭제 하시겠습니까 ?")) {
        form.action = "/treat/0406/InspctMgr_K05_Delete.do";
        form.submit();
    }
}
//////////////////////////////////////////
		...
		<div id="contents">
			...
			<h2>본문내용</h2>
				...
				<div class="title_area">
                    <p class="contents_title">기생충검사</p>
                    <ol class="location">
				...
				<div class="contents_table">
					...
					<div class="aniinfo_add_box">
						...
						<table class="aniinfo_add table_view2"  id="KS05table">
							...
							<c:forEach var="KS05INFO" items="${KS05INFO}" varStatus="st">
						    <tr id = 'tr_${KS05INFO.SEQ}'>
						    	<td>${KS05INFO.SEQ}</td>
						    	<td><input name='JJDKKS_WAY' type='text' class='textfield' value='${KS05INFO.JJDKKS_WAY}'><label for='JJDKKS_WAY'></label></td>
						    	<td><input name='JJDKKS_PART' type='text' class='textfield' value='${KS05INFO.JJDKKS_PART}'><label for='JJDKKS_PART'></label></td>
						    	<td><input name='JJDKKS_RSLT' type='text' class='textfield' value='${KS05INFO.JJDKKS_RSLT}'><label for='JJDKKS_RSLT'></label></td>
						    	<td>
						    		<c:forEach var="KS504LIST" items="${KS504LIST}" varStatus="st">
						    			<c:choose>
						    				<c:when test="${KS05INFO.JJDKKS_RSLTDC == KS504LIST.MCC_S_CODE}">
						    					<label><input type='radio' onclick='javascript:DataChk("${KS05INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS05INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' checked='checked' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:when>
						    				<c:otherwise>
						    					<label><input type='radio' onclick='javascript:DataChk("${KS05INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS05INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:otherwise>
						    			</c:choose>
						    		</c:forEach>
						    		<input type='hidden' name='JJDKKS_RSLTDC' value='${KS05INFO.JJDKKS_RSLTDC}'/>
						    	</td>
						    	<td>
									<a href='javascript:deletedata("${KS05INFO.SEQ}");' class='cancel_btn' style='width: 80px;margin-left:0px;border:solid 1px #397516;background:#4d9721;color:#fff !important;'>
										삭제
									</a>
						    	</td>
						    </tr>
						    ...
						    <tr id = 'tr_${KS05INFO.SEQ}'>
						    	<td>${KS05INFO.SEQ}</td>
						    	<td>${KS05INFO.JJDKKS_WAY}</td>
						    	<td>${KS05INFO.JJDKKS_PART}</td>
						    	<td>${KS05INFO.JJDKKS_RSLT}</td>
						    	<td>
						    		<c:forEach var="KS504LIST" items="${KS504LIST}" varStatus="st">
						    			<c:choose>
						    				<c:when test="${KS05INFO.JJDKKS_RSLTDC == KS504LIST.MCC_S_CODE}">
						    					<label><input disabled="disabled" type='radio' onclick='javascript:DataChk("${KS05INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS05INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' checked='checked' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:when>
						    				<c:otherwise>
						    					<label><input disabled="disabled" type='radio' onclick='javascript:DataChk("${KS05INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS05INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:otherwise>
						    			</c:choose>
						    		</c:forEach>
						    		<input type='hidden' name='JJDKKS_RSLTDC' value='${KS05INFO.JJDKKS_RSLTDC}'/>
