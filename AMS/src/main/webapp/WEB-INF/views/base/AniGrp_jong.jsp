...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>      
						...
						<table class="ani_list" > 
							...
							<thead>
								<tr><th scope="col">과<button class="all_btn" type="button">전체</button></th></tr>
							</thead>
							<tbody>
								<c:if test="${!empty list}">
								<c:forEach items="${list}" var="list">
								<c:if test="${gwaCnt > '1' or (gwaCnt <= '1' and !empty list.GWA_NAME)}">
								<tr>
									<td><a href="javascript:AniGrp_rowRank('jong','${list.MAC_ANICODE1}','${list.MAC_ANICODE2}','${list.MAC_ANICODE3}');"><c:if test="${!empty list.GWA_NAME}">${list.MAC_ANICODE3} - ${list.GWA_NAME}</c:if></a></td>
								</tr>
								</c:if>
								</c:forEach>
								</c:if>
						...
						<table class="ani_list" > 
							...
							<thead>
								<tr><th scope="col">종<button class="add_btn" type="button">추가</button></th></tr>
							</thead>
							<tbody>
								<c:if test="${!empty list}">
								<c:forEach items="${list}" var="list">
								<tr>
									<td><a href="#">${list.MAC_ANICODE4_SEQ} - ${list.MAC_NAME}</a></td>
								</tr>
								</c:forEach>
								</c:if>
							</tbody>
