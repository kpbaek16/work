...
function SIGNDetail(DOCNO,DOCTYPE,KJNO)
{
	...
	form.action="/sign/0701/detailSIGN.do";
	form.submit();
}
function fn_change_select()
{
	var selVal = $("#selectKind option:selected").val();
	var StatusVal = $("#selectStatus option:selected").val();
	if(selVal == '1'){
		$("#selectStatus option:eq(0)").attr('style',"display:"); //전체
		$("#selectStatus option:eq(1)").attr('style',"display:"); //임시등록
		$("#selectStatus option:eq(2)").attr('style',"display:none"); //결재대기
		$("#selectStatus option:eq(3)").attr('style',"display:"); //결재중
		$("#selectStatus option:eq(4)").attr('style',"display:"); //결재완료
		$("#selectStatus option:eq(5)").attr('style',"display:"); //반려
		$("#selectStatus option:eq(6)").attr('style',"display:none"); //공람대기
		$("#selectStatus option:eq(7)").attr('style',"display:none"); //공람완료
		if(StatusVal == '2' || StatusVal == '6' || StatusVal == '7'){
			$("#selectStatus option:eq(0)").attr("selected", "true");
		}
	}
	else if(selVal == '2'){
		$("#selectStatus option:eq(0)").attr('style',"display:");
		$("#selectStatus option:eq(1)").attr('style',"display:none");
		$("#selectStatus option:eq(2)").attr('style',"display:");
		$("#selectStatus option:eq(3)").attr('style',"display:");
		$("#selectStatus option:eq(4)").attr('style',"display:");
		$("#selectStatus option:eq(5)").attr('style',"display:");
		$("#selectStatus option:eq(6)").attr('style',"display:none");
		$("#selectStatus option:eq(7)").attr('style',"display:none");
		if(StatusVal == '1' || StatusVal == '6' || StatusVal == '7'){
			$("#selectStatus option:eq(0)").attr("selected", "true");
		}
	}
	else{
		$("#selectStatus option:eq(0)").attr('style',"display:none");
		$("#selectStatus option:eq(1)").attr('style',"display:none");
		$("#selectStatus option:eq(2)").attr('style',"display:none");
		$("#selectStatus option:eq(3)").attr('style',"display:none");
		$("#selectStatus option:eq(4)").attr('style',"display:none");
		$("#selectStatus option:eq(5)").attr('style',"display:none");
		$("#selectStatus option:eq(6)").attr('style',"display:");
		$("#selectStatus option:eq(7)").attr('style',"display:");
		if(StatusVal != '6' && StatusVal != '7'){
			$("#selectStatus option:eq(6)").attr("selected", "true");
		}
	}
}
                        ...
                        <table class="aniinfo_search" >
                            ...
                            <tbody>
								...
								<tr>
									...
									<td>
										<div class="select_box">			
											<select name="selectKind" id="selectKind" onchange="javascript:fn_change_select();">
												<option value="1" <c:if test="${param.selectKind == '1'}"> selected="selected" </c:if>>기안문서</option>
												<option value="2" <c:if test="${param.selectKind == '2'}"> selected="selected" </c:if>>결재문서</option>
												<option value="3" <c:if test="${param.selectKind == '3'}"> selected="selected" </c:if>>참조문서</option>
											</select>
										</div>
									</td>
								</tr>
								<tr>
									...
									<td>
										<div class="select_box">			
											...
											<option value="6">공람대기</option>
											<option value="7">공람완료</option>
											</select>
										</div>
									</td>
								</tr>
