...
function DataChk(SEQ,SCODE) {
	$('#tr_' + SEQ).find('input[type="hidden"]').val(SCODE);
}
...
function AddData() {
    var Rslt = document.getElementsByName('m_RsltDC');
    var Rsltname = document.getElementsByName('m_RsltDCname');
    var bum_table = document.getElementById('KS06table'); // 테이블지정
    ...
    var bum_tr, bum_td1, bum_td2, bum_td3, bum_td4, bum_td5, bum_td6;
    ...
    bum_td2.innerHTML = "<input name='JJDKJH_WAY' type='text' class='textfield' value=''><label for='JJDKJH_WAY'></label>";
    bum_td3 = document.createElement('TD');    // Td를 생성한다.
    bum_td3.innerHTML = "<input name='JJDKJH_PART' type='text' class='textfield' value=''><label for='JJDKJH_PART'></label>";
    bum_td4 = document.createElement('TD');    // Td를 생성한다.
    bum_td4.innerHTML = "<input name='JJDKJH_RSLT' type='text' class='textfield' value=''><label for='JJDKJH_RSLT'></label>";
    var a;
    for (var i = 0; i < Rslt.length; ++i) {
        if (i == 0) {
            a = "<label><input type='radio' onclick='javascript:DataChk(\"" + No + "\",\"" + Rslt[i].value + "\");' name='RSLTDC_" + No + "' value='" + Rslt[i].value + "' class='search_radio' />" + Rsltname[i].value + "</label>";
        }
        else {
            a += "<label><input type='radio' onclick='javascript:DataChk(\"" + No + "\",\"" + Rslt[i].value + "\");' name='RSLTDC_" + No + "' value='" + Rslt[i].value + "' class='search_radio' />" + Rsltname[i].value + "</label>";
        }
    }
    a += "<input type='hidden' name='JJDKJH_RSLTDC' value=''/>";
    bum_td5 = document.createElement('TD');    // Td를 생성한다.
    bum_td5.innerHTML = a;
    bum_td6 = document.createElement('TD');    // Td를 생성한다.
    bum_td6.innerHTML = "<a href='javascript:deletedata(\"" + No + "\");' class='cancel_btn' style='width: 80px;margin-left:0px;border:solid 1px #397516;background:#4d9721;color:#fff !important;'>삭제</a>";
    ...
    bum_tr.appendChild(bum_td6);      // Tr에 td를 하나 추가한다.
}
function fn_MSG(){
...
function prc_Save() {
    ...
    if (confirm("저장 하시겠습니까 ?")) {
        form.action = "/treat/0406/InspctMgr_K06_Save.do";
        form.submit();
    }
}
function prc_delete() {
    ...
    if (confirm("삭제 하시겠습니까 ?")) {
        form.action = "/treat/0406/InspctMgr_K06_Delete.do";
        form.submit();
    }
}
//////////////////////////////////////////
		...
		<div id="contents">
			...
			<h2>본문내용</h2>
				...
				<div class="title_area">
                    <p class="contents_title">잠혈반응검사</p>
                    <ol class="location">
				...
				<c:forEach var="KS504LIST" items="${KS504LIST}" varStatus="st">
					<input type="hidden" name="m_RsltDC" value="${KS504LIST.MCC_S_CODE}" />
        			<input type="hidden" name="m_RsltDCname" value="${KS504LIST.MCC_S_NAME}" />
				</c:forEach>
				<!-- contents-->
				<div class="contents_table">
					...
					<div class="aniinfo_add_box">
						...
						<table class="aniinfo_add table_view2"  id="KS06table">
							...
							<c:forEach var="KS06INFO" items="${KS06INFO}" varStatus="st">
						    <tr id = 'tr_${KS06INFO.SEQ}'>
						    	<td>${KS06INFO.SEQ}</td>
						    	<td><input name='JJDKJH_WAY' type='text' class='textfield' value='${KS06INFO.JJDKJH_WAY}'><label for='JJDKJH_WAY'></label></td>
						    	<td><input name='JJDKJH_PART' type='text' class='textfield' value='${KS06INFO.JJDKJH_PART}'><label for='JJDKJH_PART'></label></td>
						    	<td><input name='JJDKJH_RSLT' type='text' class='textfield' value='${KS06INFO.JJDKJH_RSLT}'><label for='JJDKJH_RSLT'></label></td>
						    	<td>
						    		<c:forEach var="KS504LIST" items="${KS504LIST}" varStatus="st">
						    			<c:choose>
						    				<c:when test="${KS06INFO.JJDKJH_RSLTDC == KS504LIST.MCC_S_CODE}">
						    					<label><input type='radio' onclick='javascript:DataChk("${KS06INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS06INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' checked='checked' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:when>
						    				<c:otherwise>
						    					<label><input type='radio' onclick='javascript:DataChk("${KS06INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS06INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:otherwise>
						    			</c:choose>
						    		</c:forEach>
						    		<input type='hidden' name='JJDKJH_RSLTDC' value='${KS06INFO.JJDKJH_RSLTDC}'/>
						    	</td>
						    	<td>
									<a href='javascript:deletedata("${KS06INFO.SEQ}");' class='cancel_btn' style='width: 80px;margin-left:0px;border:solid 1px #397516;background:#4d9721;color:#fff !important;'>
										삭제
									</a>
						    	</td>
						    </tr>
						    ...
						    <tr id = 'tr_${KS06INFO.SEQ}'>
						    	<td>${KS06INFO.SEQ}</td>
						    	<td>${KS06INFO.JJDKJH_WAY}</td>
						    	<td>${KS06INFO.JJDKJH_PART}</td>
						    	<td>${KS06INFO.JJDKJH_RSLT}</td>
						    	<td>
						    		<c:forEach var="KS504LIST" items="${KS504LIST}" varStatus="st">
						    			<c:choose>
						    				<c:when test="${KS06INFO.JJDKJH_RSLTDC == KS504LIST.MCC_S_CODE}">
						    					<label><input disabled="disabled" type='radio' onclick='javascript:DataChk("${KS06INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS06INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' checked='checked' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:when>
						    				<c:otherwise>
						    					<label><input disabled="disabled" type='radio' onclick='javascript:DataChk("${KS06INFO.SEQ}","${KS504LIST.MCC_S_CODE}");' name='RSLTDC_${KS06INFO.SEQ}' value='${KS504LIST.MCC_S_CODE}' class='search_radio' />${KS504LIST.MCC_S_NAME}</label>
						    				</c:otherwise>
						    			</c:choose>
						    		</c:forEach>
						    		<input type='hidden' name='JJDKJH_RSLTDC' value='${KS06INFO.JJDKJH_RSLTDC}'/>
