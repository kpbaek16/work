...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
						...
						<table class="ani_list" > 
							...
							<thead>
								<tr><th scope="col">목<button class="add_btn" type="button">추가</button></th></tr>
							</thead>
						    <tbody>
								<c:if test="${!empty list}">
								<c:forEach items="${list}" var="list">
								<tr>
									<td><a href="javascript:AniGrp_rowRank('gwa','${list.MAC_ANICODE1}','${list.MAC_ANICODE2}','');">${list.MAC_ANICODE2} - ${list.MAC_NAME}</a></td>
								</tr>
								</c:forEach>
								</c:if>
							</tbody>
