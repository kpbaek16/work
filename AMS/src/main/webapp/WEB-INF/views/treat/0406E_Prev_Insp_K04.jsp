...
function DataChk(SEQ) {
	var Rslt = document.getElementsByName('RSLT_' + SEQ);
	var KS05VAL = "";
    for(var i=0;i<Rslt.length;++i)
    {
    	 if(Rslt[i].checked == true){
    		 KS05VAL += Rslt[i].value +",";
    	 }
    }
    KS05VAL = KS05VAL.slice(0, -1);
	$('#tr_' + SEQ).find('input[type="hidden"]').val(KS05VAL);
}
...
function AddData() {
    var Rslt = document.getElementsByName('m_Rslt');
    var bum_table = document.getElementById('KS04table'); // 테이블지정
    ...
    bum_td2.innerHTML = "<input name='JJDKHS_PART' type='text' class='textfield' value=''><label for='JJDKHS_PART'></label>";
    bum_td3 = document.createElement('TD');    // Td를 생성한다.
    bum_td3.innerHTML = "<input name='JJDKHS_DSKYUN' type='text' class='textfield' value=''><label for='JJDKHS_DSKYUN'></label>";
    bum_td4 = document.createElement('TD');    // Td를 생성한다.
    bum_td4.style.lineHeight = "1.5"; 
    var a;
    for(var i=0;i<Rslt.length;++i)
    {
    	if(i == 0){
    		a = "<input type='checkbox' onchange='javascript:DataChk(\""+ No +"\");' name = 'RSLT_"+ No +"' value = '" + Rslt[i].value + "'> <label for='RSLT_"+ No +"'>" + Rslt[i].value + "&nbsp</label>";
    	}
    	else{
    		a += "<input type='checkbox' onchange='javascript:DataChk(\""+ No +"\");' name = 'RSLT_"+ No +"' value = '" + Rslt[i].value + "'> <label for='RSLT_"+ No +"'>" + Rslt[i].value + "&nbsp</label>";
    	}
    ...
    bum_td5.innerHTML = "<input type='hidden' name='JJDKHS_RSLT' value=''/><a href='javascript:deletedata(\"" + No + "\");' class='cancel_btn' style='width: 80px;margin-left:0px;border:solid 1px #397516;background:#4d9721;color:#fff !important;'>삭제</a>";
...
function prc_Save() {
    ...
    if (confirm("저장 하시겠습니까 ?")) {
        form.action = "/treat/0406/InspctMgr_K04_Save.do";
        form.submit();
    }
}
function prc_delete() {
    ...
    if (confirm("삭제 하시겠습니까 ?")) {
        form.action = "/treat/0406/InspctMgr_K04_Delete.do";
        form.submit();
    }
}
//////////////////////////////////////////
		...
		<div id="contents">
			...
			<h2>본문내용</h2>
				...
				<div class="title_area">
                    <p class="contents_title">항생제감수성검사</p>
                    <ol class="location">
				...
				<c:forEach var="KS503LIST" items="${KS503LIST}" varStatus="st">
					<input type="hidden" name="m_Rslt" value="${KS503LIST.MCC_S_CODE}" />
				</c:forEach>
				<!-- contents-->
				<div class="contents_table">
					...
					<div class="aniinfo_add_box">
						...
						<table class="aniinfo_add table_view2"  id="KS04table">
							...
							<c:forEach var="KS04INFO" items="${KS04INFO}" varStatus="st">
							<c:set var="KS503INFO" value="${fn:split(KS04INFO.JJDKHS_RSLT,',')}" />
						    <tr id = 'tr_${KS04INFO.SEQ}'>
						    	<td>${KS04INFO.SEQ}</td>
						    	<td><input name='JJDKHS_PART' type='text' class='textfield' value='${KS04INFO.JJDKHS_PART}'><label for='JJDKHS_PART'></label></td>
						    	<td><input name='JJDKHS_DSKYUN' type='text' class='textfield' value='${KS04INFO.JJDKHS_DSKYUN}'><label for='JJDKHS_DSKYUN'></label></td>
						    	<td style='line-height: 1.5;'>
						    		<c:forEach var="KS503LIST" items="${KS503LIST}" varStatus="st">
						    			<c:set var="Chk" value="" />
						    			<c:forEach var="s1" items="${KS503INFO}" varStatus="st">
						    			<c:if test="${s1 == KS503LIST.MCC_S_CODE}">
						    				<c:set var="Chk" value="checked='checked'" />
						    			</c:if>
						    			</c:forEach>
						    			<input type='checkbox' ${Chk} onchange='javascript:DataChk("${KS04INFO.SEQ}");' name = 'RSLT_${KS04INFO.SEQ}' value = '${KS503LIST.MCC_S_CODE}'> <label for='RSLT_${KS04INFO.SEQ}'>${KS503LIST.MCC_S_CODE}&nbsp</label>
						    		</c:forEach>
						    	</td>
						    	<td>
						    		<input type='hidden' name='JJDKHS_RSLT' value='${KS04INFO.JJDKHS_RSLT}'/>
									<a href='javascript:deletedata("${KS04INFO.SEQ}");' class='cancel_btn' style='width: 80px;margin-left:0px;border:solid 1px #397516;background:#4d9721;color:#fff !important;'>
										삭제
									</a>
						    	</td>
						    </tr>
						    ...
						    <tr id = 'tr_${KS04INFO.SEQ}'>
						    	<td>${KS04INFO.SEQ}</td>
						    	<td>${KS04INFO.JJDKHS_PART}</td>
						    	<td>${KS04INFO.JJDKHS_DSKYUN}</td>
						    	<td style='line-height: 1.5;'>
						    		<c:forEach var="KS503LIST" items="${KS503LIST}" varStatus="st">
						    			<c:set var="Chk" value="" />
						    			<c:forEach var="s1" items="${KS503INFO}" varStatus="st">
						    			...
						    			<input disabled="disabled" type='checkbox' ${Chk} onchange='javascript:DataChk("${KS04INFO.SEQ}");' name = 'RSLT_${KS04INFO.SEQ}' value = '${KS503LIST.MCC_S_CODE}'> <label for='RSLT_${KS04INFO.SEQ}'>${KS503LIST.MCC_S_CODE}&nbsp</label>
						    		</c:forEach>
						    	</td>
						    	<td>
						    		<input type='hidden' name='JJDKHS_RSLT' value='${KS04INFO.JJDKHS_RSLT}'/>
